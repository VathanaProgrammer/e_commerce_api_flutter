...
@section('content')
                ...
                <x-widget title="Edit Product: {{ $product->name }}">
                    <form id="form_update_product" enctype="multipart/form-data" method="POST"
                        ...
                        <div class="row g-4">
                                ...
                                <div class="premium-card mb-4 section-box">
                                    ...
                                    <div class="p-4">
                                        <div id="descriptionLines" class="mb-3">
                                            @forelse ($product->descriptionLines as $line)
                                            ...
                                            @empty
                                                <div class="description-line mb-2">
                                                    <div class="input-group-premium">
                                                        <i class="bi bi-dash input-icon"></i>
                                                        <input type="text" name="description_lines[]" class="form-control" placeholder="Add characteristic...">
                                                        <button type="button" class="btn-remove-line remove-line">
                                                            <i class="bi bi-x"></i>
                                                        </button>
                                                    </div>
                                ...
                                <div class="premium-card section-box">
                                    ...
                                    <div class="p-4">
                                        <div class="image-upload-wrapper">
                                            <label class="upload-zone {{ $product->image_url ? 'd-none' : '' }}" for="productImage">
                                                <div class="upload-icon">
                                                    <i class="bi bi-cloud-arrow-up-fill"></i>
                                                </div>
                                                <p class="m-0">Click to change current image</p>
                                                <input type="file" name="image" id="productImage" class="d-none" accept="image/*">
                                            </label>
                                            <div class="image-preview-area text-center {{ $product->image_url ? '' : 'd-none' }}" id="previewContainer">
                                                <img id="imagePreview" src="{{ $product->image_url ? asset($product->image_url) : '' }}" class="img-fluid rounded-4 shadow-sm" style="max-height: 300px;">
                                                <div class="mt-3">
                                                    <button type="button" class="btn btn-sm btn-outline-primary rounded-pill px-3 me-2" onclick="$('#productImage').click()">
                                                        <i class="bi bi-pencil-square me-1"></i> Change
                                                    </button>
                                                    <button type="button" class="btn btn-sm btn-danger rounded-pill px-3" id="removePreview">
                            ...
                            <div class="col-lg-5">
                                ...
                                <div class="premium-card mb-4 section-box">
                                    ...
                                    <div class="p-4">
                                                ...
                                                <div class="attribute-pill-group mb-4" data-id="{{ $attr->id }}">
                                                    ...
                                                    <div class="attribute-options">
                                                        @foreach ($attr->values as $val)
                                                            @php
                                                                $isConnected = $product->variants->some(fn($v) => $v->attributeValues->contains('id', $val->id));
                                                            @endphp
                                        ...
                                        <button type="button" id="generateVariants" class="btn btn-premium-primary w-100">
                                            <i class="bi bi-magic me-2"></i> Regenerate Variants
                                        </button>
                                    </div>
                                </div>
                                                    ...
                                                    <div class="d-flex justify-content-between align-items-center mb-3">
                                                        <div class="variant-badges-container">
                                                            @foreach ($variant->attributeValues as $attrValue)
                                                                <span class="variant-badge-pill">{{ $attrValue->attribute->name }}: {{ $attrValue->value }}</span>
                                ...
                                <div class="premium-card section-box">
                                    ...
                                    <div class="p-4">
                                        @php $currentDiscount = $product->discounts->first(); @endphp
                                        <div class="mb-3">
                                            ...
                                            <div class="input-group-premium">
                                                ...
                                                <input type="text" name="discount[name]" class="form-control" 
                                                    value="{{ $currentDiscount?->name }}" placeholder="Discount Name">
                                            </div>
                                        </div>
                                            ...
                                            <div class="col-8">
                                                ...
                                                <div class="input-group-premium">
                                                    ...
                                                    <input type="number" step="0.01" name="discount[value]" class="form-control" 
                                                        value="{{ $currentDiscount?->value }}" placeholder="0.00">
                                                </div>
                                            </div>
                                            <div class="col-4">
                                                <select name="discount[is_percentage]" class="form-select border-2 py-2">
                                                    <option value="1" {{ $currentDiscount?->is_percentage ? 'selected' : '' }}>%</option>
                                                    <option value="0" {{ $currentDiscount && !$currentDiscount->is_percentage ? 'selected' : '' }}>$</option>
                                                </select>
                                            </div>
                                        </div>
                                        <div class="mt-3 text-end d-flex align-items-center justify-content-end">
                                            ...
                                            <div class="form-check form-switch m-0">
                                                <input class="form-check-input" type="checkbox" name="discount[active]" value="1" 
                                                    {{ ($currentDiscount?->active ?? true) ? 'checked' : '' }}>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div class="mt-5 pt-4 border-top d-flex justify-content-end gap-3 align-items-center">
                            ...
                            <button type="submit" id="btnUpdateProduct" class="btn btn-premium-action rounded-pill px-5 py-2 fw-bold">
                                <i class="bi bi-cloud-arrow-up-fill me-1"></i> Update Product Details
                            </button>
                        </div>
                    </form>
                </x-widget>
    ...
    <style>
        ...
        .premium-card:hover { box-shadow: 0 10px 30px rgba(0, 0, 0, 0.05); border-color: #cbd5e1; transform: translateY(-2px); }
        ...
        .card-header-premium i { color: var(--premium-primary); }

        .form-label-premium {
            font-weight: 600; font-size: 0.85rem; color: #64748b; margin-bottom: 8px; display: block; text-transform: uppercase; letter-spacing: 0.5px;
        }

        .input-group-premium { position: relative; display: flex; align-items: center; }

        .input-icon { position: absolute; left: 14px; color: #94a3b8; font-size: 1.1rem; z-index: 10; }

        .input-group-premium .form-control, .input-group-premium .form-select {
            padding-left: 42px; border: 2px solid var(--premium-border); border-radius: 14px; padding-top: 10px; padding-bottom: 10px; font-weight: 500;
        }

        .input-group-premium .form-control:focus { border-color: var(--premium-primary); box-shadow: 0 0 0 4px rgba(102, 126, 234, 0.1); }

        .btn-remove-line {
            position: absolute; right: 8px; background: none; border: none; color: #f43f5e; font-size: 1.5rem; display: flex; align-items: center; justify-content: center; padding: 0; opacity: 0.6; transition: all 0.2s; z-index: 11;
        }

        .btn-remove-line:hover { opacity: 1; transform: scale(1.2); }

        .status-toggle-card { background: #fff; border: 2px solid var(--premium-border); border-radius: 14px; padding: 10px 16px; transition: all 0.2s; }

        .status-toggle-card:hover { border-color: var(--premium-primary); background: #fdfefe; }

        .image-upload-wrapper {
            border: 2px dashed #cbd5e1; border-radius: 20px; padding: 10px; transition: all 0.3s; cursor: pointer; min-height: 180px; display: flex; align-items: center; justify-content: center;
        }

        .image-upload-wrapper:hover { border-color: var(--premium-primary); background: rgba(102, 126, 234, 0.02); }

        .upload-zone { text-align: center; width: 100%; height: 100%; padding: 40px; cursor: pointer; }

        .upload-icon { font-size: 3rem; color: var(--premium-primary); margin-bottom: 10px; transition: transform 0.3s; }
        ...
        .attr-label {
            background: white; border: 2px solid var(--premium-border); padding: 8px 18px; border-radius: 30px; cursor: pointer; font-weight: 600; font-size: 0.9rem; transition: all 0.2s; user-select: none;
        }

        .attr-checkbox input:checked + .attr-label {
            background: var(--premium-primary); border-color: var(--premium-primary); color: white; box-shadow: 0 4px 12px rgba(102, 126, 234, 0.3);
        }

        .variant-premium-card { background: white; border: 2px solid var(--premium-border); border-radius: 16px; padding: 20px; margin-bottom: 16px; transition: all 0.3s; }

        .variant-badge-pill {
            display: inline-block; background: #f1f5f9; color: #475569; font-weight: 700; font-size: 0.75rem; padding: 5px 12px; border-radius: 8px; margin-right: 6px; margin-bottom: 6px; text-transform: uppercase; border: 1px solid #e2e8f0;
        }

        .btn-premium-primary { background: var(--premium-primary); color: white; border-radius: 14px; padding: 12px; font-weight: 700; border: none; transition: all 0.3s; }

        .btn-premium-primary:hover { transform: translateY(-2px); box-shadow: 0 5px 15px rgba(102, 126, 234, 0.4); }

        .btn-premium-action {
            background: linear-gradient(135deg, var(--premium-primary) 0%, var(--premium-secondary) 100%); color: white; border: none; box-shadow: 0 4px 15px rgba(102, 126, 234, 0.3); transition: all 0.3s;
        }

        .btn-premium-action:hover { transform: translateY(-2px); box-shadow: 0 8px 25px rgba(102, 126, 234, 0.5); color: white; }

        .btn-light-muted { background: #f1f5f9; color: #64748b; border: none; }
        ...
        $(function() {
            ...
            $('#generateVariants').click(function() {
                ...
                $('.attribute-pill-group').each(function() {
                    ...
                    let attrName = $(this).find('.form-label-premium').first().text();
                    let vals = [];
                    $(this).find('.attribute-check:checked').each(function() {
                        vals.push({ id: $(this).val(), name: $(this).next('.attr-label').text(), attrName: attrName });
                    });
                    if (vals.length) selected[attrId] = vals;
                });

                let keys = Object.keys(selected);
                if (!keys.length) { toastr.warning('Please select at least one attribute first!'); return; }
                        ...
                        <div class="variant-premium-card animate__animated animate__fadeInUp">
                            ...
                            <div class="row g-3">
                                <div class="col-6">
                                    ...
                                    <div class="input-group-premium">
                                        <i class="bi bi-barcode input-icon"></i>
                                        <input type="text" name="variants[${index}][sku]" class="form-control form-control-sm" placeholder="SKU-PRD">
                                    </div>
                                </div>
                                <div class="col-6">
                                    <label class="form-label-premium">Price ($)</label>
                                    <div class="input-group-premium">
                                        <i class="bi bi-currency-dollar input-icon"></i>
                                        <input type="number" step="0.01" name="variants[${index}][price]" class="form-control form-control-sm" required>
                                    </div>
                                </div>
                            </div>
                            ${hiddenInputs}
            ...
            $(document).on('click', '.remove-variant', function() {
                ...
                if ($('#variantsSection .variant-premium-card').length === 0) { $('#noVariantsPlaceholder').show(); }
            });
            ...
            $('#form_update_product').on('submit', function(e) {
                ...
                $.ajax({
                    ...
                    success: function(res) {
                        toastr.success(res.msg || 'Success! Product has been updated.');
                    ...
                    error: function(err) {
                        ...
                        } else { toastr.error('An error occurred while updating.'); }
                    }
                });
            });
        });
