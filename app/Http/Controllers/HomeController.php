...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        $currentWeekRevenue = \App\Models\Transaction::where('status', \App\Enums\TransactionStatus::Completed)
            ->where('created_at', '>=', $now->copy()->subDays(7))
            ->sum('total_sell_price');

        // Previous week revenue
        $previousWeekRevenue = \App\Models\Transaction::where('status', \App\Enums\TransactionStatus::Completed)
        ...
        if ($previousWeekRevenue > 0) {
            $growth = (($currentWeekRevenue - $previousWeekRevenue) / $previousWeekRevenue) * 100;
        }
